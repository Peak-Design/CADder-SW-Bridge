...
You need SolidWorks 2022 or newer (for the interop assemblies) and the .NET
SDK. The build fails with a clear message when it cannot find the interops;
point it at them with `-p:SolidWorksApiDir=...`.
...
Install (or update) **STEPper NEXT**; its `rig/` subpackage registers the
**SW To Blender** panel in the 3D View sidebar automatically. Point the panel
at a `.rig.json`; the STEP file must sit beside it, exactly as the exporter
wrote the pair.
