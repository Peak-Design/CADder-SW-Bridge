; Inno Setup script for the SW To Blender add-in.
;
; Build: tools\Make-Release.ps1 runs this after the Release build and passes
; the version. By hand:
;   ISCC.exe /DVersion=0.1.0 installer\SwToBlender.iss
;
...
#endif
#define AppName "SW To Blender"
#define Publisher "Peak Design"
#define Source "..\src\Peak.SwToBlender\bin\Release"
...
AppPublisherURL=https://github.com/Peak-Design/SW-To-Blender
DefaultDirName={commonpf}\{#Publisher}\{#AppName}
DefaultGroupName={#AppName}
DisableProgramGroupPage=yes
OutputDir=..\dist
OutputBaseFilename=SwToBlender-{#Version}-setup
...
LicenseFile=..\LICENSE
UninstallDisplayIcon={app}\Peak.SwToBlender.dll
WizardStyle=modern

[Files]
Source: "{#Source}\Peak.SwToBlender.dll"; DestDir: "{app}"; Flags: ignoreversion
...
Filename: "{#RegAsm}"; Parameters: """{app}\Peak.SwToBlender.dll"" /codebase"; \
  StatusMsg: "Registering the add-in with SolidWorks..."; Flags: runhidden waituntilterminated

[UninstallRun]
Filename: "{#RegAsm}"; Parameters: """{app}\Peak.SwToBlender.dll"" /unregister"; \
...
begin
  ...
  begin
    MsgBox('Close SolidWorks before you install SW To Blender.', mbError, MB_OK);
    Result := False;
  end;
end;
...
begin
  ...
  if SolidWorksRunning() then
  begin
    MsgBox('Close SolidWorks before you remove SW To Blender.', mbError, MB_OK);
    Result := False;
  end;
end;
