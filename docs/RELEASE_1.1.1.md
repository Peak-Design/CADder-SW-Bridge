...
Better rigs from your mates, Refresh Model when there is a scene to
refresh, and a warning when CADder and CADder Bridge do not match. Use it
with [CADder 1.1.1](https://github.com/Peak-Design/CADder/releases/latest).
This release replaces 1.1.0.

## Install

1. Close SolidWorks.
2. Run `CADder-Bridge-1.1.1-setup.exe` below. It replaces the version you
   have.
3. Update CADder in Blender to 1.1.1 too.

## New

- **The add-in asks before it sends to a CADder that does not match.**
  CADder Bridge and CADder work together when the first two numbers of
  their versions are the same: CADder Bridge 1.1.1 works with CADder
  1.1.0. When they do not match, **Send to Blender** and **Refresh Model**
  show both versions, which one to update and a link to its download,
  before anything is sent. **Continue** sends all the same, and **Abort**
  sends nothing. See
  [Versions](https://github.com/Peak-Design/CADder-SW-Bridge#versions).
